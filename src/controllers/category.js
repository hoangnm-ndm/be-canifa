...
export const updateCategory = async (req, res, next) => {
	try {
		...
		return res.status(200).json({
			message: successMessages?.UPDATE_SUCCESS || "Cap nhat danh muc thanh cong!",
			data,
		});
	} catch (error) {
		next(error);
...
export const softDeleteCategory = async (req, res, next) => {
	try {
		const data = await Category.findByIdAndUpdate(req.params.id, { isHidden: true }, { new: true });
		if (!data) {
			return res.status(400).json({
...
export const deleteCategory = async (req, res, next) => {
	try {
		...
		if (req.params.id === "669b4d58ced9a2e70375e250") {
			return res.status(400).json({
		...
		await Promise.all(
			productsToUpdate.map(async (product) => {
				product.category = "669b4d58ced9a2e70375e250"; // ID của danh mục mặc định
				await product.save();
			})
		);
